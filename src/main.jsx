...
function App() {
  ...
  const [indexMembership, setIndexMembership] = useState({ ksc100: [], ksc30: [] });
  ...
  useEffect(() => { fetch('/api/symbols').then((response) => response.json()).then((payload) => { setSymbols(payload.symbols); setIndexMembership(payload.indexMembership || { ksc100: [], ksc30: [] }); }); }, []);
  useEffect(() => {
    let active = true;
  ...
  const comparisonSymbols = symbols.filter((item) => {
    if (!item.toLowerCase().includes(comparisonQuery.toLowerCase())) return false;
    if (activeFilter === 'ksc100') return indexMembership.ksc100.includes(item);
    if (activeFilter === 'ksc30') return indexMembership.ksc30.includes(item);
    return true;
  });
  ...
  async function compareSelected() {
    ...
    for (let i = 0; i < selectedSymbols.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(batch.map(async (item) => {
        try {
          const response = await fetch(`/api/analysis/${item}?mode=${mode}&limit=200`);
      ...
      if (i + batchSize < selectedSymbols.length) await delay(1000);
    }
    setComparisonResults(results.filter((item) => item.signal));
    setComparisonErrors(results.filter((item) => !item.signal));
  ...
  async function predictSelected() {
    ...
    for (let i = 0; i < selectedSymbols.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(batch.map(async (item) => {
        try {
          const response = await fetch(`/api/predict/${item}?mode=${mode}&limit=200`);
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          return await response.json();
        } catch (error) {
          return { symbol: item, predictionError: error.message };
        }
      }));
      results.push(...batchResults);
      if (i + batchSize < selectedSymbols.length) await delay(500);
    }
    setPredictionResults(results.filter((item) => item.signal));
  ...
  function selectCurrentFilter() {
    let allMatchedSymbols;
    if (activeFilter === 'ksc100') {
      allMatchedSymbols = comparisonSymbols;
      setSelectedSymbols(indexMembership.ksc100.filter((s) => allMatchedSymbols.includes(s)));
    } else if (activeFilter === 'ksc30') {
      allMatchedSymbols = comparisonSymbols;
      setSelectedSymbols(indexMembership.ksc30.filter((s) => allMatchedSymbols.includes(s)));
    } else {
      setSelectedSymbols([...comparisonSymbols]);
    }
  }
  function selectAllCurrentFilter() {
    if (activeFilter === 'ksc100') {
      setSelectedSymbols([...indexMembership.ksc100]);
    } else if (activeFilter === 'ksc30') {
      setSelectedSymbols([...indexMembership.ksc30]);
    } else {
      setSelectedSymbols([...comparisonSymbols]);
    }
  }
  const predictionRankedResults = [...predictionResults]
    .map((item) => ({ ...item, edgeScore: item.signal?.edgeScore ?? 0, signalLabel: item.signal?.directionalSignal ?? 'NO_TRADE' }))
  ...
  const validatedAccuracy = (item) => item?.validation?.metrics?.directionalAccuracy ?? 0.5;
  const rankedResults = [...comparisonResults]
    .map((item) => ({ ...item, validationScore: (item.signal.edgeScore ?? 0) * validatedAccuracy(item) }))
  ...
  return <main>
    ...
    <section className="compare panel"><div className="panel-head"><div><span className="eyebrow">UNIVERSE RANKING</span><h3>Compare symbols</h3></div><span className="muted">{selectedSymbols.length} selected</span></div><div className="compare-controls"><div className="filter-buttons"><button className={activeFilter === 'all' ? 'active' : ''} onClick={() => { selectFilter('all'); setSelectedSymbols([...comparisonSymbols]); }}>All</button><button className={activeFilter === 'ksc100' ? 'active' : ''} onClick={() => { selectFilter('ksc100'); setSelectedSymbols([...indexMembership.ksc100]); }}>KSC-100</button><button className={activeFilter === 'ksc30' ? 'active' : ''} onClick={() => { selectFilter('ksc30'); setSelectedSymbols([...indexMembership.ksc30]); }}>KSC-30</button><button className="active" onClick={selectAllCurrentFilter} disabled={comparisonSymbols.length === 0}>Select {activeFilter === 'all' ? 'All' : activeFilter.toUpperCase()}</button></div><input value={comparisonQuery} onChange={(event) => setComparisonQuery(event.target.value)} placeholder="Filter listed symbols" /><button className="active" disabled={!selectedSymbols.length || comparisonLoading} onClick={compareSelected}>{comparisonLoading ? 'Analyzing...' : 'Rank selected'}</button><button className="active predict-button" disabled={!selectedSymbols.length || predictionLoading} onClick={predictSelected} style={{background:'#db8c56'}}>{predictionLoading ? 'Predicting...' : 'Quick Predict'}</button><button className="help-button" onClick={() => setShowExplanations((value) => !value)}>{showExplanations ? 'Hide explanations' : 'Explain results'}</button></div>{showExplanations && <div className="explanations"><div><strong>BUY</strong><span>The model sees a strong positive pattern, but this is a paper signal, not an order.</span></div><div><strong>SELL</strong><span>The model sees a strong negative pattern. It does not mean short-selling is available.</span></div><div><strong>NO_TRADE</strong><span>The evidence is too weak or a safety check blocked the candidate.</span></div><div><strong>Edge</strong><span>How strong the expected move is compared with recent price variability. Higher is not automatically safer.</span></div><div><strong>Liquidity</strong><span>Whether the stock normally trades enough shares for the configured minimum. BLOCK means it may be difficult to enter or exit.</span></div><div><strong>VaR / CVaR</strong><span>Estimated bad-day loss and average loss in the worst 1% of historical outcomes.</span></div><div><strong>Spread unavailable</strong><span>PSX public data gives price and volume, but not live bid and ask prices. Trading cost cannot be measured, so real execution stays blocked.</span></div><div><strong>Confidence</strong><span>Confidence in this model pattern, not a guarantee that the price will move as predicted.</span></div></div>}<div className="symbol-picks">{comparisonSymbols.map((item) => <label key={item} className={selectedSymbols.includes(item) ? 'picked' : ''}><input type="checkbox" checked={selectedSymbols.includes(item)} onChange={() => setSelectedSymbols((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item])} />{item}</label>)}</div>{comparisonResults.length > 0 && <div className="candidate-summary">{strongestBuy ? <div className="strongest actionable"><span className="eyebrow">STRONGEST ACTIONABLE PAPER BUY</span><strong>{strongestBuy.symbol}</strong><b>PSX price {strongestBuy.quote.close.toFixed(2)}</b><span>edge {strongestBuy.signal.edgeScore.toFixed(3)} · confidence {formatPercent(strongestBuy.signal.confidence)}</span><small>Paper signal only · live execution remains blocked</small></div> : <div className="strongest"><span className="eyebrow">STRONGEST ACTIONABLE PAPER BUY</span><strong>NONE</strong><span>No selected symbol passed the BUY gates.</span></div>}{blockedPositive ? <div className="strongest blocked"><span className="eyebrow">HIGHEST POSITIVE EDGE BLOCKED</span><strong>{blockedPositive.symbol}</strong><b>PSX price {blockedPositive.quote.close.toFixed(2)}</b><span>edge {blockedPositive.signal.edgeScore.toFixed(3)} · {blockedPositive.signal.signal} · {blockedPositive.signal.liquidity.pass ? 'liquidity passed' : 'liquidity blocked'}</span><small>{blockedPositive.signal.reasons.at(-1)}</small></div> : <div className="strongest"><span className="eyebrow">HIGHEST POSITIVE EDGE BLOCKED</span><strong>NONE</strong><span>No positive but blocked candidate in this selection.</span></div>}</div>}{comparisonResults.length > 0 && <div className="ranking">{rankedResults.map((item, index) => <div key={item.symbol}><span>#{index + 1}</span><strong>{item.symbol}</strong><b>PSX {item.quote.close.toFixed(2)}</b><b className={item.signal.signal === 'BUY' ? 'buy-text' : item.signal.signal === 'SELL' ? 'sell-text' : ''}>{item.signal.signal}</b><small>validated edge {item.validationScore.toFixed(3)} · direction {formatPercent(validatedAccuracy(item))} · backtest {item.validation?.verdict?.replace(/_/g, ' ') || '--'} ({item.validation?.portfolio?.totalTrades ?? 0} trades) · VaR {(item.risk.var99 * 100).toFixed(2)}% · liquidity {item.signal.liquidity.pass ? 'PASS' : 'BLOCK'}</small></div>)}</div>}</section>
      {comparisonErrors.length > 0 && <div className="error-summary"><span className="eyebrow">DATA UNAVAILABLE</span><h3>{comparisonErrors.length} symbol(s) lacked sufficient data</h3><p>These symbols had fewer than the required 55+ historical bars for accurate backtesting.</p>{comparisonErrors.map((item) => <div key={item.symbol} className="error-item"><strong>{item.symbol}</strong><span>{item.comparisonError}</span></div>)}</div>}
      {predictionResults.length > 0 && <div>
        <div className="candidate-summary">{predictionRankedResults.length > 0 ? <div className="strongest actionable"><span className="eyebrow">TOP PREDICTION</span><strong>{predictionRankedResults[0].symbol}</strong><b>PSX price {predictionRankedResults[0].quote?.close?.toFixed(2)}</b><span>edge {predictionRankedResults[0].edgeScore.toFixed(3)} · {predictionRankedResults[0].signalLabel}</span><small>Paper signal · {predictionRankedResults[0].prediction?.horizonBars ?? 5} bars · {predictionRankedResults[0].prediction?.confidence ? formatPercent(predictionRankedResults[0].prediction.confidence) : '--'}</small></div> : <div className="strongest"><span className="eyebrow">TOP PREDICTION</span><strong>NONE</strong><span>No symbol passed prediction gates.</span></div>}</div>
        {predictionRankedResults.length > 0 && <div className="ranking">{predictionRankedResults.map((item, index) => <div key={item.symbol}><span>#{index + 1}</span><strong>{item.symbol}</strong><b>PSX {item.quote?.close?.toFixed(2)}</b><b className={item.signalLabel === 'BUY' ? 'buy-text' : item.signalLabel === 'SELL' ? 'sell-text' : ''}>{item.signalLabel}</b><small>edge {item.edgeScore.toFixed(3)} · direction {formatPercent(item.prediction?.expectedReturn ?? 0)} · {item.prediction?.model?.replace(/_/g, ' ') || '--'} · VaR {(item.risk?.var99 * 100).toFixed(2)}% · liquidity {item.signal?.liquidity?.pass ? 'PASS' : 'BLOCK'} · {item.prediction?.extremeForecast ? 'EXTREME' : ''}</small></div>)}</div>}
      </div>}
